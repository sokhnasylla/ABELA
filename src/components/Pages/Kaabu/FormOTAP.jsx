import React, { Component } from 'react';
import Card from '@mui/material/Card';
import CardContent from '@mui/material/CardContent';
import CardMedia from '@mui/material/CardMedia';
import Typography from '@mui/material/Typography';
import { withStyles } from '@mui/styles';
import { getDeviceImageService, getDeviceVendeurService } from './kaabu.service';
import { CircularProgress } from '@mui/material';
import { Button, Modal, Row } from 'react-bootstrap';
import MapsWidget2 from './MapsWidget2'
import {FaSearchLocation } from 'react-icons/fa';
import MapsWidget from './MapsWidget';


class DeviceCard extends Component {
    
    state = {
        telephone: this.props.telephone,
        latitude: this.props.latitude,
        longitude: this.props.longitude,
        username: this.props.username,
        subscriber: null,
        deviceLoading: false,
        imageLoading: false,
        showModal: false,
        imageData: null,
        imageDefault: "data:text/xml;base64,"
      };

      
  getDevice() {
    this.setState({
        deviceLoading: true,
     });

    getDeviceVendeurService(this.state.telephone).then((result) => {
      if(result) {
        if (result.success) {
          console.log("----getDeviceVendeurService----------", result);
          this.setState({
              imageLoading: true,
          });
           getDeviceImageService(result.data.subscriber.device.id).then(dataUri => {
            console.log("----getDeviceImageService----------", dataUri);
            if (dataUri === "data:text/xml;base64,") {
              console.log("----------------------");
              this.setState({imageLoading: true, imageData: this.state.imageDefault });
            } else {
              this.setState({imageLoading: true, imageData: dataUri });
            }
            console.log("------------IMAGE", dataUri);
          }).finally(() => {
            this.setState({
              imageLoading: false,
             });
          });
          this.setState({
            subscriber: result.data.subscriber,
           });
        } else {
          //this.alertService.showNotificationAlertError(result.message || 'Une erreur s\'est produite');
        }
      }
    })
    .finally(() => {
      this.setState({
        deviceLoading: false,
       });
    });
  }

  componentDidMount() {
    console.log("----CONTRUCTOR-------", this.props);
    this.getDevice()
  }

  render() {
    console.log("----RENDER-------", this.props);
    if(this.state.deviceLoading){
      return (
        <div style={{ marginTop: "50px", width: "100%", justifyContent: "space-around", display: "inline-flex"}}>
          <CircularProgress color="primary" size={60} />
        </div>)
    }
    return this.state.subscriber && (
      <div style={{marginTop: "50px"}}>
        <div style={{display: 'flex', alignItems: "center", justifyContent: "space-between"}}>
          {this.state.imageLoading ? <CircularProgress color="primary" size={50} /> : <img style={{padding: "20px", height: "130px"}} src={this.state.imageData} alt={this.state.subscriber.device.model} />}
          <Button onClick={() => this.setState({showModal: true})} style={{ height: "40px", marginTop: "50px", fontWeight: 'bold', backgroundColor: "black", borderColor: " black" }}><FaSearchLocation style={{color: "white", fontSize:'125%', marginRight: "10px"}}/>Localiser le vendeur</Button>
        </div>
        
        <CardContent style={{marginTop: "7px", borderTop: "1px silver solid"}}>
          <Typography gutterBottom variant="h5" component="div">
            {this.state.subscriber.device.model}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>IMEI:</strong> {this.state.subscriber.imei}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>MSISDN:</strong> {this.state.subscriber.msisdn}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>IMSI:</strong> {this.state.subscriber.imsi}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>DUAL SIM:</strong> {this.state.subscriber.device.sim_number > 1 ? "YES" : "NO"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>LTE:</strong> {this.state.subscriber.device.lte=="Y" ? "YES" : "NO"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>NFC:</strong> {this.state.subscriber.device.nfc=="Y" ? "YES" : "NO"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>OS:</strong> {this.state.subscriber.device.os}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            <strong>Year:</strong> {this.state.subscriber.device.year}
          </Typography>
        </CardContent>

        <Modal show={this.state.showModal} onHide={() => this.setState({showModal: false})}>
          <Modal.Body style={{ height: '500px', width: '500px' }}>
              {/* <MapsWidget2 lat={parseFloat(this.state.latitude)} lng={parseFloat(this.state.longitude)}/> */}
              <MapsWidget lat={parseFloat(this.state.latitude)} lng={parseFloat(this.state.longitude)} vendorName={this.state.username} />
          </Modal.Body>
        </Modal>
      </div>
    );
  }
}

export default DeviceCard;
